      ...
      <div class="box">
        ...
        <form class="hm_kids" v-on:submit.prevent>
          ...
          <div v-for="item in children_essential" :key="item.instance_id" class="hm_row">
            ...
            <span class="hm_center">{{ item.target_state }}</span>
            <button :disabled="accessLevel < 1" @click="set_target(item, 'up')">up</button>
            <button :disabled="accessLevel < 1" @click.stop="openPopup(item)">down</button>
          </div>
          <div v-if="currentlyOpenPopupId" class="popup-container" id="popup" style="background-color: rgba(0,0,0,0.4);">
            <div class="popup-content">
              <h2>Warning!</h2>
              <p>{{ currentlyOpenPopupId.instance_id }}</p>
              <p>This container is essential to PCS operations. Do you wish to proceed?</p>
              <button @click="set_target(currentlyOpenPopupId, 'down'); closePopup()">Bring down container</button>
              <button @click="closePopup()">Return</button>
            </div>
          </div>
        </form>
      </div>
...
<script>
  export default {
    ...
    methods: {
      ...
      openPopup(container) {
        this.currentlyOpenPopupId = container;
      ...
      closePopup() {
        ...
        this.currentlyOpenPopupId = null;
      },
    },
  }
